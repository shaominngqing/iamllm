...
DEFAULT_TIMEOUT_FALLBACK_TEXT = "||".join(
    [
        "【超时兜底】我知道你很急，但你先别急。这个请求还在路上，稍后再试一次。",
        "【超时兜底】连接没有断，只是这次响应暂时迷路了。过会儿再发一次。",
        "【超时兜底】请求已经成功送达，但没能在时限内完成。稍后重试一下。",
        "【超时兜底】这次不是你的网络问题，是响应时间刚好用完了。稍后再来。",
        "【超时兜底】服务正在短暂整理思路，这一轮先到这里。稍后重新发起请求。",
        "【超时兜底】你的问题没有丢，只是回复速度暂时切到了省电模式。稍后再试。",
    ]
)
