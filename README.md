...
- 无需刷新：请求队列会自动同步新问题，并在页面休眠时降低刷新频率
- 大上下文治理：列表只传轻量摘要；详情默认突出当前任务，系统指令、历史消息、工具定义和工具结果按需展开
- 统一控制台：概览、请求队列、API 密钥、接入指南、自动回复和服务设置都在一个单页后台中完成
...
- 图片输入：网页上传 JPEG/PNG/WebP；API 支持 OpenAI 图片内容、Claude base64/URL 图片块和 Gemini inlineData/fileData
- 模型能力：可在管理端维护展示名称、简介、服务状态和技能列表
- 工具调用：接收 Chat Completions、Responses、Claude 和 Gemini 的函数工具定义，并返回对应协议的调用块
...
3. 让调用者从自己熟悉的客户端发起请求；新请求会自动出现在控制台队列，无需刷新。
4. 在请求队列中查看完整上下文、图片和工具定义，并连续发送回复 chunk。
5. 使用 `/chat` Playground 做本地连通性、上下文、图片和流式回复测试。

## 请求队列的连续处理方式

- 待回答队列按先来后到排列，发送答案后会自动打开下一条，不需要反复点列表。
- 每条请求都会生成不超过 88 个字符的短标题，长 system prompt、工具输出或结构化数据不会再挤满列表。
- “重点”视图只展开当前要处理的输入，其余内容按“系统与开发者指令 / 对话历史 / 工具过程 / 可调用工具”分组折叠；“完整”视图仍可按协议顺序检查全部消息。
- 折叠内容只在点开时创建页面内容，单条超长文本也会先显示摘要，可继续展开或复制原文。该机制对 OpenAI、Claude、Gemini 和其他兼容客户端统一生效。
